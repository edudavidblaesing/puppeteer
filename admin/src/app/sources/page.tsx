                ...
                <div className="bg-white dark:bg-gray-900 rounded-xl shadow border border-gray-200 dark:border-gray-800 overflow-hidden">
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                            {sources
                                .filter(s => s.code !== 'original' && s.code !== 'manual' && s.code !== 'og')
                                .map((source) => (
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <a href={source.base_url} target="_blank" rel="noopener noreferrer" className="text-xs text-gray-500 hover:text-primary-500 font-mono">
                                                        {source.code}
                                                    </a>
                                                </div>
                                            </div>
                                                ...
                                                {SCOPES.map(scope => {
                                                    ...
                                                    return (
                                                        <label key={scope.id} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border cursor-pointer transition-colors ${!source.is_active ? 'opacity-50 cursor-not-allowed' :
                                                            ...
                                                            />
                                                            <Icon className={`w-3.5 h-3.5 ${enabled ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400'}`} />
                                                            <span className={`text-sm ${enabled ? 'text-primary-900 dark:text-primary-100 font-medium' : 'text-gray-500'}`}>
                                                                {scope.label}
                                                            </span>
                                                        </label>
                                                    );

const { services: { artistService } } = require('@social-events/shared');
...
const matchArtists = async (req, res) => {
    try {
        ...
        const result = await callScraper('/scrape/artists/match', 'POST', { dryRun, minConfidence });
        res.json(result);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
const enrichArtists = async (req, res) => {
    try {
        const result = await callScraper('/scrape/artists/enrich', 'POST', {});
        res.json(result);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
const enrichArtist = async (req, res) => {
    try {
        const result = await callScraper(`/scrape/artists/${req.params.id}/enrich`, 'POST', {});
        res.json(result);
    } catch (error) {

...
async function run() {
    try {
        ...
        const venueData = {
            ...
            city: 'Berlin',
            country: 'Germany',
            latitude: 52.5200, // Service expects numbers, Controller/Schema handles coercion
            longitude: 13.4050
        };
        ...
        console.log('Venue Audit Log:', venueLog.rows[0]);
        if (venueLog.rows[0].performed_by === mockUser.id) {
            console.log('✅ Venue Audit Performed By matches mock user');
        } else {
            console.error('❌ Venue Audit Performed By MISMATCH');
        }

        console.log('\n--- Testing Event Creation Audit ---');
        // eventService.create(data, artistsList, user)
        const eventData = {

...
class ArtistService {
    ...
    async match(options) {
        return await matchAndLinkArtists(options);
    }
    ...
    async enrichOne(id) {
        const artistRes = await this.pool.query('SELECT name, country FROM artists WHERE id = $1', [id]);
        ...
        const scrapedRes = await this.pool.query(`
        ...
        return { artist: await this.findById(id), source_data: details };
    }
}

...
async function run() {
    try {
        ...
        for (const row of res.rows) {
            ...
            try {
                // If changes is null or empty, just clear the flag
                ...
                if (fields.length > 0) {
                    ...
                    // Just reset the flag if no specific fields
                    await pool.query('UPDATE scraped_events SET has_changes = false WHERE id = $1', [row.scraped_id]);
                }
                console.log(`Success.`);
            } catch (err) {
